Shader "Custom/ForceFieldWithAlpha"
{
    Properties {
        ...
        _IntersectionFactor("Intersection Factor", float) = 1
    ...
    SubShader
    {
        ...
        Pass {
            ...
            V2F vert(Input input)
            {
                ...
                output.positionVS = mul(UNITY_MATRIX_MV,input.positionOS);
            ...
            float4 frag(V2F input):SV_TARGET
            {
                ...
                fresnel *= color;
                       ...
                       // Eye Depth from the depth texture at the same screen position of the current fragment
                ...
                // Eye Depth of the current pixel/fragment
                float fragmentLinearEyeDepth = -input.positionVS.z;

                // Difference between the 2 depths
                float depthDifference = sceneLinearEyeDepth - fragmentLinearEyeDepth;

                // To be able to control the width of the intersection
                depthDifference *= _IntersectionFactor;

                float _IntersectionValue = saturate(1 - depthDifference);
                
			    //lerp between the main color and the fresnel color depends on the fresnel value
                return lerp(_MainColor, _FresnelColor, fresnel + _IntersectionValue );
            }
